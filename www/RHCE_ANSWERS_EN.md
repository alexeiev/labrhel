...
Install the required packages, create a static inventory file, and configure ansible.cfg on the control node.
...
### Configuration: /home/greg/ansible/ansible.cfg
...
## 2. Configure YUM Repositories (yum_repo.yml)

Create yum_repo.yml to configure EX294_BASE and EX294_STREAM repositories on all nodes. GPG is disabled for the lab (enabled in the real exam).
...
Install php and mariadb on dev, test and prod. Install "RPM Development Tools" on dev. Update all packages on dev. Use a separate play for each task.
...
## 4. Install Ansible Content Collections

Install the collections from the provided URLs into /home/greg/ansible/mycollection.
...
Create a roles directory and a requirements.yml file to download the balancer and phpinfo roles.
...
## 6. Use RHEL SELinux System Role (selinux.yml)

Create a playbook that runs on all managed nodes, uses the selinux role with policy targeted and state enforcing.
...
## 7. Create Apache Role

Create an offline role named apache: install httpd, enable firewalld with HTTP, host a web page using a Jinja2 template.
...
## 8. Use Balancer and PHPInfo Roles (roles.yml)

Create roles.yml to run the phpinfo role on webservers and the balancer role on balancers group.
...
## 9. Create and Use Logical Volume (lv.yml)

Create a logical volume "data" in volume group "research" with 1500 MiB (fallback to 800 MiB). Format with ext4. Handle missing VG.
...
## 10. Generate Hosts File from Template (hosts.yml)

Download the hosts.j2 template. Create a playbook that generates /etc/myhosts on the dev group with all inventory host information.
...
## 11. Replace /etc/issue (issue.yml)

Replace /etc/issue with "Development" on dev, "Test" on test, and "Production" on prod.
...
## 12. Web Content Directory (webcontent.yml)

Create /webdev directory owned by webdev group with set-gid, create index.html with "Development", and symlink to /var/www/html/webdev.
...
## 13. Hardware Report (hwreport.yml)

Download hwreport.empty, save as /root/hwreport.txt, and fill in HOST, MEMORY, BIOS, DISK_SIZE_VDA, DISK_SIZE_VDB. Show "NONE" if info is missing.
...
## 14. Create Vault (locker.yml)

Create an encrypted variable file locker.yml with pw_developer and pw_manager. Store the vault password in secret.txt.
...
## 15. Create User Accounts (users.yml)

Download user_list.yml. Create users: developers in devops group on dev/test, managers in opsmgr group on prod. SHA512 password, 30 days max expiry.

### Download variable file
...
## 16. Rekey Vault File (salaries.yml)

Download salaries.yml and rekey it with a new password. The file must remain encrypted.
...
## 17. Create Cron Job (cron.yml)

Create a cron job for user natasha on dev group nodes. Every 2 minutes execute: logger "EX294 in progress".
